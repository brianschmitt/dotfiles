# Generated by Powerlevel10k configuration wizard on 2025-07-30 at 13:11 EDT.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    twilio                  # Your custom Twilio segment (see below)
    time                    # current time
  )
...
function prompt_twilio() {
  # Only show if a Twilio-related variable is set
  if [[ -n "$TWILIO_ENV_NAME" ]]; then
    p10k segment -b 5 -f 7 -i '📞' -t "$TWILIO_ENV_NAME"
  elif [[ -n "$TWILIO_ACCOUNT_SID" ]]; then
    # Show last 4 of SID if no env name is set
    p10k segment -b 4 -f 7 -i '📞' -t "${TWILIO_ACCOUNT_SID: -4}"
  fi
}